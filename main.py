...
async def fetch_sitemap(url: str) -> str:
    """Fetch sitemap.xml content asynchronously using aiohttp."""
    ...
    if sitemap_tags:
        for sitemap in sitemap_tags:
            ...
            if loc is not None and loc.text:
                # Recursively fetch and parse each sitemap in the index
                sitemap_xml = asyncio.run(fetch_sitemap(loc.text))
                urls.extend(parse_sitemap(sitemap_xml))
    else:
        # Standard sitemap: extract all <loc> tags
        loc_tags = root.findall('.//ns:loc', namespace)
        urls = [loc.text.strip() for loc in loc_tags if loc.text]
    return urls

async def get_pydantic_ai_docs_urls() -> List[str]:
    """Asynchronously fetch and parse the Pydantic AI docs sitemap."""
    sitemap_url = "https://ai.pydantic.dev/sitemap.xml"
    xml_content = await fetch_sitemap(sitemap_url)
    if not xml_content:
        print("Failed to fetch sitemap content.")
        return []
